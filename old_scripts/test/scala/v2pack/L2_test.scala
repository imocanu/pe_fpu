...
class L2_PE_test_generate(dut: L2_PE) extends PeekPokeTester(dut) {
...
class L2_PE_test_simple(dut: L2_PE) extends PeekPokeTester(dut) {
  ...
  println(" ----->  [Simple test] : ") 
    ...
    test_in_0 = "b"+"00000000000000000101101010000010"
    test_in_1 = "b"+"00000000000000000101101010000010"
    test_out  = "b"+"01111111111111101010100000010000"
    poke(dut.io.rounding, "b000".U)
    step(10)
    poke(dut.io.tininess, "b1".U(1.W))
    step(10)
    poke(dut.io.in_0, test_in_0.U(32.W))  
...
class L2_PE_test extends ChiselFlatSpec with Matchers {
    val vcdName = "L2_PE_test"
    val targetDir = "diagram/" + vcdName
    val unittestsDir = "diagram/unit_tests/test"


    "run L2_PE_simple" should "pass" in {
    chisel3.iotesters.Driver.execute(Array(
      ...
      "--output-file", vcdName),
      () => new L2_PE ) { c =>
      new L2_PE_test_simple (c)
    } should be (true)
  }

    // "run L2_PE_test_generate" should "pass" in {
    // chisel3.iotesters.Driver.execute(Array(
    //   "--fint-write-vcd",
    //   "--backend-name", "firrtl",
