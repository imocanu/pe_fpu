...
class test_random_MULT_FP32(dut: MultPE) extends PeekPokeTester(dut) {
...
class test_random_MULT_INT(dut: MultPE) extends PeekPokeTester(dut) {
  println(" ----->  TEST RANDOM with [ MULT -> INT ]")
  println(" ----->  [Run test] : ") 
...
class MultPE_test_manual(dut: MultPE) extends PeekPokeTester(dut) {
