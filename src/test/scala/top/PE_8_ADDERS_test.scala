...
class PE_8_ADDERS_test_INT_L2(dut: PE_8_ADDERS) extends PeekPokeTester(dut) {
...
class PE_8_ADDERS_test_manual(dut: PE_8_ADDERS) extends PeekPokeTester(dut) {
    ...
    println(" ----->  Manual TEST for [ PE -> INT ]")
    println(" ----->  [Run test] : ") 
    ...
    step(100)
    // expect(dut.io.out_0, "b01001000010110011001000000000000".U(32.W))
    // step(100)
...
class PE_8_ADDERS_test extends ChiselFlatSpec with Matchers {
    val vcdName = "PE_8_ADDERS_test"
    val targetDir = "diagram/" + vcdName


    "run PE_8_ADDERS_test_manual" should "pass" in {
