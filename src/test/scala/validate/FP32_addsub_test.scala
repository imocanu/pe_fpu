...
class FP32_random_SUB_FP32(dut: FP32_addsub) extends PeekPokeTester(dut) {
...
class FP32_random_ADD_FP32(dut: FP32_addsub) extends PeekPokeTester(dut) {
...
class FP32_random_SUB_INT(dut: FP32_addsub) extends PeekPokeTester(dut) {
...
class FP32_random_ADD_INT(dut: FP32_addsub) extends PeekPokeTester(dut) {
...
class AddSubPE_test_simple(dut: FP32_addsub) extends PeekPokeTester(dut) {
  ...
  test_out  = "b"+"01000010111011000000000000000000"  // −65534
  poke(dut.io.in_0, test_in_0.U(32.W)) 
  poke(dut.io.in_1, test_in_1.U(32.W))
  step(10)
  expect(dut.io.out, test_out.U(32.W))
...
class FP32_addsub_test extends ChiselFlatSpec with Matchers {
    val vcdName = "FP32_addsub_test"
  ...
  //     "--output-file", vcdName),
  //     () => new FP32_addsub ) { c =>
  //     new AddSubPE_test_simple (c)
  //   } should be (true)
  // }

    "run FP32_random_ADD_INT" should "pass" in {
    chisel3.iotesters.Driver.execute(Array(
      "--fint-write-vcd",
      "--backend-name", "firrtl",
    ...
    chisel3.iotesters.Driver.execute(Array(
      ...
      "--output-file", vcdName),
      () => new FP32_addsub ) { c =>
      new FP32_random_SUB_INT (c)
    } should be (true)
  }

    "run FP32_random_ADD_FP32" should "pass" in {
    chisel3.iotesters.Driver.execute(Array(
      "--fint-write-vcd",
      "--backend-name", "firrtl",
      ...
      "--output-file", vcdName),
      () => new FP32_addsub ) { c =>
      new FP32_random_ADD_FP32 (c)
    } should be (true)
  }

    "run FP32_random_SUB_FP32" should "pass" in {
    chisel3.iotesters.Driver.execute(Array(
      "--fint-write-vcd",
      "--backend-name", "firrtl",
